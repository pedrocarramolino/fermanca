...
export default async function StatisticsPage() {
  ...
  // El desglose se agrupa por bloque (ver categoryBreakdown), así que hereda
  // el nombre del bloque más reciente de cada categoría — correcto si el
  // usuario lo renombró, pero no traducible por sí solo. Para las 4
  // categorías de sistema, este mapa lo sustituye por el nombre de la
  // categoría (vía su slug estable) en el idioma de quien la ve.
  const categoryList = await new SupabaseCategoryRepository(supabase).listAvailable(userId);
  const categoryNames = new Map(
    categoryList
      .filter((category) => category.kind === "system")
      .map((category) => [category.id, tCategories(category.slug)] as const),
  );

  const now = new Date();
  const weekly = weeklySeries(sessions, 12, now);
  const monthly = monthlySeries(sessions, 12, now);
  const breakdown = categoryBreakdown(sessions).map((stat) => ({
    ...stat,
    name: categoryNames.get(stat.categoryId as CategoryId) ?? stat.name,
  }));
  const categories = foldIntoOthers(breakdown, 7, t("categoryOthers"));
