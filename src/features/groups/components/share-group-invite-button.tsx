...
/** Mismo patrón que ShareInviteButton (amigos): share nativo si el
 * navegador lo soporta, y si no, un menú con WhatsApp/Facebook/copiar
...
export function ShareGroupInviteButton({
  ...
  const [joinUrl] = useState(() =>
    typeof window === "undefined"
      ? ""
      : `${window.location.origin}/community/groups/join/${inviteCode}`,
  );

  const shareText = t("groupInviteText", { name: groupName });

  async function handleNativeShare() {
    try {
      await navigator.share({ title: groupName, text: shareText, url: joinUrl });
    } catch {
